...
- AOP
>AOP (Aspect-oriented programming) is an approach to programming that allows global >properties of a program to determine how it is compiled into an executable program. >AOP can be used with object-oriented programming ( OOP ). An aspect is a subprogram >that is associated with a specific property of a program.
- DAO
...
>
- MVC
>MVC (Model View Controller) is one of three ASP.NET programming models. MVC is a framework for building web applications using a MVC design: The Model represents the application core (for instance a list of database records). The View displays the data (the database records).
